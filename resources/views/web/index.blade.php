...
<div class="row">
    ...
    <img src="{{asset('storage/images/'.$product["images"][0]["file"])}}" alt="#">
    <div class="button">
        @if (Auth::check())
        <bottom id_product="{{ $product->id }}"  class="btn add_to_cart"><i class="lni lni-cart"></i> Add to Cart</bottom>
        @else
        <a href="{{ route("web/login") }}" class="btn"><i class="lni lni-cart"></i> Add to Cart</a>

        @endif
    </div>
    </div>
     <div class="product-info">
    <span class="category">{{ $product->cateogry }}</span>
    <h4 class="title">
